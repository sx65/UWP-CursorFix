...
std::string getWindowTitle(HWND handle)
{
    if (handle)
    {
        ...
        GetWindowTextW(handle, title, size);
...
void init()
{
    ...
    {
        std::cout << "Failed to find the Roblox window. Make sure it is open." << std::endl;
        return;
    }
...
void check()
{
    while (true)
    {
        ...
        if (GetForegroundWindow() == robloxHWND && isEnabled)
        {
...
int main()
{
    ...
    if (mode == 1 || mode == 2)
    {
        std::cout << "Enter the button for toggle (e.g., A, B, F1, etc.): ";
        std::cin >> toggle_button_str;
        toggle_button = buttonenum_to_button(string_to_button(toggle_button_str));

        std::ofstream toggleFile("_ToggleButton.txt");
        toggleFile << toggle_button_str;
        toggleFile.close();
    ...
    std::cout << "Invalid option, please restart the software." << std::endl;
    return 0;
}